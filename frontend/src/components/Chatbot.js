...
const Chatbot = () => {
    ...
    useEffect(() => {
        if (isOpen && messages.length === 0) {
            addBotMessage("👋 Hello! I'm your MeroSewa Complaint Assistant. How can I help you today?\n\n• File a complaint\n• Check complaint status\n• Get help");
        }
    }, [isOpen]);

    // Auto-reshow preview message after 10 seconds if dismissed
    useEffect(() => {
    ...
    const processUserInput = (input) => {
        ...
        if (conversationState === 'idle') {
            if (lowerInput.includes('file') || lowerInput.includes('complaint') || lowerInput.includes('report') || lowerInput.includes('problem')) {
                setConversationState('ask_name');
                addBotMessage("Let's file your complaint. First, what is your full name?");
            } else if (lowerInput.includes('status') || lowerInput.includes('check') || lowerInput.includes('track')) {
        ...
        if (conversationState === 'ask_phone') {
            ...
            if (phoneRegex.test(input.replace(/\s/g, ''))) {
                setComplaintData(prev => ({ ...prev, phone: input.replace(/\s/g, '') }));
                setConversationState('ask_email');
                addBotMessage("What is your email address? (Type 'skip' if you don't want to provide)");
            } else {
        ...
        if (conversationState === 'ask_manual_location') {
            // Check if input contains comma (coordinates) or is just a location name
        ...
        if (conversationState === 'ask_address') {
            ...
            addBotMessage("What type of problem do you want to report?\n\n• Water\n• Electricity\n• Road\n• Garbage\n• Street Light\n• Drainage\n• Pollution\n• Others");
            return;
        }
        ...
        if (conversationState === 'ask_complaint_type') {
            ...
            addBotMessage("What is the priority level?\n\n• Low\n• Medium\n• High\n• Emergency");
            return;
        }
        ...
        if (conversationState === 'ask_priority') {
            ...
            if (validPriorities.includes(lowerInput)) {
                setComplaintData(prev => ({ ...prev, priority: input.charAt(0).toUpperCase() + input.slice(1).toLowerCase() }));
        ...
        if (conversationState === 'ask_description') {
            ...
            addBotMessage("When did this incident occur? (Format: YYYY-MM-DD or type 'today')");
            return;
        }
        ...
        if (conversationState === 'ask_incident_date') {
            ...
            setComplaintData(prev => {
                ...
                // Show summary with the complete data
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    try {
                        ...
                        const locationCoords = `${latitude.toFixed(4)}, ${longitude.toFixed(4)}`;
                        setComplaintData(prev => ({
                            ...prev,
                ...
                (error) => {
                    addBotMessage("Could not detect location. Please enter manually:\nFormat: Area name or latitude, longitude");
                    setConversationState('ask_manual_location');
                }
            );
        } else {
            addBotMessage("Location detection not supported. Please enter manually:\nFormat: Area name or latitude, longitude");
            setConversationState('ask_manual_location');
        }
    };
    ...
    const submitComplaint = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                addBotMessage("❌ Failed to submit complaint. Please try again or use the complaint form.");
                resetConversation();
            }
        } catch (error) {
            console.error('Chatbot submission error:', error);
            addBotMessage("❌ Error submitting complaint. Please try the complaint form instead.");
            resetConversation();
        }
    };
    ...
    const checkComplaintStatus = async (complaintId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                addBotMessage("❌ Complaint not found. Please check the ID and try again.");
            }
        } catch (error) {
                ...
                >
                    ...
                    {conversationState === 'ask_location_method' && (
                        <div className="chatbot-quick-actions">
                            <button onClick={() => handleQuickAction('location')} className="quick-btn">
                    ...
                    {conversationState === 'idle' && messages.length > 1 && (
                        <div className="chatbot-quick-actions">
                            <button onClick={() => handleQuickAction('file')} className="quick-btn">
                                📝 File Complaint
                            </button>
                            <button onClick={() => handleQuickAction('status')} className="quick-btn">
                                🔍 Check Status
                            </button>
                        </div>
                    )}
